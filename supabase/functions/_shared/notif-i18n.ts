...
const texts: Record<string, Record<string, { title: string; body: string | ((...args: any[]) => string) }>> = {
  ...
  friend_accepted: {
    ...
    de: {
      ...
      body: (name: string) => `${name} hat deine Freundschaftsanfrage akzeptiert! 🎉`,
    },
  },
  challenge_completed: {
    fr: {
  ...
  boost_completed: {
    fr: {
      ...
      body: (name: string, refunded: boolean) =>
        `${name} a réussi le défi que tu lui as offert ! ${refunded ? "Tu as été remboursé." : ""}`,
    },
    en: {
      title: "Gifted challenge completed! 🎉",
      body: (name: string, refunded: boolean) =>
        `${name} completed the challenge you gifted! ${refunded ? "You've been refunded." : ""}`,
    },
    de: {
      title: "Geschenkte Challenge geschafft! 🎉",
      body: (name: string, refunded: boolean) =>
        `${name} hat die geschenkte Challenge geschafft! ${refunded ? "Du wurdest erstattet." : ""}`,
    },
  },
  challenge_failed: {
    fr: {
  ...
  social_challenge: {
    fr: {
      ...
      body: (name: string, bet: number, sessions: number, months: number) =>
        `@${name} t'offre un défi de ${bet}€ — ${sessions}x/sem pendant ${months} mois`,
    },
    en: {
      title: "You've been gifted a challenge! 🎁",
      body: (name: string, bet: number, sessions: number, months: number) =>
        `@${name} gifted you a €${bet} challenge — ${sessions}x/week for ${months} months`,
    },
    de: {
      title: "Du hast eine Challenge geschenkt bekommen! 🎁",
      body: (name: string, bet: number, sessions: number, months: number) =>
        `@${name} schenkt dir eine ${bet}€ Challenge — ${sessions}x/Woche für ${months} Monate`,
    },
  },
  challenge_accepted: {
    ...
    de: {
      title: "Challenge angenommen! 🔥",
      body: (name: string) => `@${name} hat deine Challenge angenommen! Los geht's`,
    },
  },
  challenge_declined: {
    fr: {
      title: "Défi refusé",
      body: (name: string, refunded: boolean) =>
        refunded ? `@${name} a refusé le défi. Tu as été remboursé.` : `@${name} a refusé le défi.`,
    },
    en: {
      title: "Challenge declined",
      body: (name: string, refunded: boolean) =>
        refunded ? `@${name} declined the challenge. You've been refunded.` : `@${name} declined the challenge.`,
    },
    de: {
      title: "Challenge abgelehnt",
      body: (name: string, refunded: boolean) =>
        refunded
          ? `@${name} hat die Challenge abgelehnt. Du wurdest erstattet.`
          : `@${name} hat die Challenge abgelehnt.`,
    },
  },
  referral_reward: {
    fr: {
      title: "Bonus parrainage disponible 🪙",
      body: (name: string, coins: number) => `${name} a validé son défi. Clique pour récupérer ${coins} pièces.`,
    },
    en: {
      title: "Referral bonus available 🪙",
      body: (name: string, coins: number) => `${name} completed their challenge. Tap to claim ${coins} coins.`,
    },
    de: {
      title: "Empfehlungsbonus verfügbar 🪙",
      body: (name: string, coins: number) =>
        `${name} hat die Challenge geschafft. Tippe, um ${coins} Münzen zu erhalten.`,
    },
  },
  referral_signup: {
    fr: {
      title: "Nouveau filleul ! 🎉",
      body: (name: string) => `@${name} s'est inscrit grâce à toi. Récupère tes 50 pièces !`,
    },
    en: { title: "New referral! 🎉", body: (name: string) => `@${name} signed up thanks to you. Claim your 50 coins!` },
    de: {
      title: "Neue Empfehlung! 🎉",
      body: (name: string) => `@${name} hat sich dank dir angemeldet. Hol dir deine 50 Münzen!`,
    },
  },
  gym_saved: {
    fr: {
      title: "Salle enregistrée ! 📍",
      body: (name: string) => `Ta salle "${name}" a été enregistrée. Tu recevras un rappel à chaque visite.`,
    },
    en: {
      title: "Gym saved! 📍",
      body: (name: string) => `Your gym "${name}" has been saved. You'll get a reminder on each visit.`,
    },
    de: {
      title: "Gym gespeichert! 📍",
      body: (name: string) => `Dein Gym "${name}" wurde gespeichert. Du erhältst bei jedem Besuch eine Erinnerung.`,
    },
  },
  coin_gift: {
    fr: {
      title: "Tu as reçu des coins ! 🎁",
      body: (name: string, amount: number, msg?: string) =>
        msg
          ? `@${name} t'a envoyé ${amount} pièces : "${msg}"`
          : `@${name} t'a envoyé ${amount} pièces. Bonne chance !`,
    },
    en: {
      title: "You received coins! 🎁",
      body: (name: string, amount: number, msg?: string) =>
        msg ? `@${name} sent you ${amount} coins: "${msg}"` : `@${name} sent you ${amount} coins. Keep it up!`,
    },
    de: {
      title: "Du hast Coins erhalten! 🎁",
      body: (name: string, amount: number, msg?: string) =>
        msg
          ? `@${name} hat dir ${amount} Münzen geschickt: "${msg}"`
          : `@${name} hat dir ${amount} Münzen geschickt. Weiter so!`,
    },
  },
};
