...
import { Stripe as CapStripe, ApplePayEventsEnum } from "@capacitor-community/stripe";
  ...
  const handleApplePay = async () => {
    ...
    try {
      ...
      if (!err?.message?.toLowerCase().includes("cancel")) {
        toast({ title: err.message || "Apple Pay failed", variant: "destructive" });
      }
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full max-h-[calc(90vh-5rem)] overflow-hidden">
      <div className="flex-1 overflow-y-auto space-y-4 px-4 pb-2">
        ...
        {!isFree && (
          <>
            {/* Apple Pay button — native via @capacitor-community/stripe */}
            {applePayAvailable && (
              <>
                <Button
                  type="button"
