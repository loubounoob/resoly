...
const UsernameGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  // Check if user already has a real username (with retry for race condition)
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
    ...
    const checkProfile = async () => {
      ...
      if (!data) {
        // Profile not yet created by trigger — retry up to 5 times
        if (retryCount.current < 5) {
          retryCount.current++;
          setTimeout(checkProfile, 800);
          return;
        }
        // After retries, show the username form (profile will be created by upsert)
        setHasUsername(false);
        return;
      }

      // If username is auto-generated (user_xxxx), treat as no username
